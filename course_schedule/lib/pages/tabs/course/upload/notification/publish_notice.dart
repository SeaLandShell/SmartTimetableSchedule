...
class PublishNoticePage extends StatefulWidget {
  ...
  final Notice? notice;
  const PublishNoticePage({super.key,required this.schedule,this.notice});

  @override
  State<PublishNoticePage> createState() => _PublishNoticePageState();
}
...
class _PublishNoticePageState extends State<PublishNoticePage> {
  ...
  void initState() {
    ...
    if(widget.notice!=null){
      _controller.document = Document.fromJson(jsonDecode(widget.notice!.content!));
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          ActionIconButton(
            ...
            tooltip: "发布通知",
            onPressed: () async {
              widget.notice==null ? DialogUtil.showConfirmDialog(context, "确认发布通知？", () async {
                String json = jsonEncode(_controller.document.toDelta().toJson());
                Notice notice=Notice();
                notice.author=widget.schedule.teacherName;
                notice.content=json;
                notice.type=0;
                notice.courseId=widget.schedule.courseId;
                notice.releaseTime="";
                final resp = await HttpUtil.client.post("/cschedule/notices",
                    data: notice.toJson());
                final data = HttpUtil.getDataFromResponse(resp.toString());
                if(data!=null){
                  Util.showToastCourse("发布成功", context);
                  Navigator.pop(context);
                }
              }) : DialogUtil.showConfirmDialog(context, "确认修改该条通知？", () async {
                String json = jsonEncode(_controller.document.toDelta().toJson());
                Notice notice=Notice();
                ...
                notice.type=0;
                notice.courseId=widget.schedule.courseId;
                final resp = await HttpUtil.client.post("/cschedule/notices/update",
                    data: notice.toJson());
                final data = HttpUtil.getDataFromResponse(resp.toString());
                if(data!=null){
                  Util.showToastCourse("修改成功", context);
      ...
      child: Column(
        children: [
          TextFormField(
            ...
            enabled: false,
            decoration: const InputDecoration(labelText: '     课程通知',),
            validator: RequiredValidator(errorText: '通知名必填'),
          ),
        ],
      ),
    );
