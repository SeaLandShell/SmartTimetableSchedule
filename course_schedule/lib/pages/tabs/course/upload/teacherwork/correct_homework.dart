...
class CorrectHomeworkPage extends StatefulWidget {
  final Schedule schedule;
  final Upload upload;
  const CorrectHomeworkPage({super.key,required this.schedule,required this.upload});

  @override
  State<CorrectHomeworkPage> createState() => _CorrectHomeworkPageState();
}

class _CorrectHomeworkPageState extends State<CorrectHomeworkPage> {
  /// 内容距状态栏的高度
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          ActionIconButton(
            ...
            tooltip: "打回",
            onPressed: () {
              DialogUtil.showConfirmDialog(context, "确认打回重新提交吗？", () async {
                Upload submitUpload = widget.upload;
                String json = jsonEncode(_controller.document.toDelta().toJson());
                submitUpload.content = json;
                submitUpload.appraise=_appraiseController.text.trim();
                submitUpload.criticism=_criticismController.text.trim();
                submitUpload.score=int.parse(_scoreController.text.trim());
                submitUpload.review="2";
                if((await ApiClientSchdedule.correct(submitUpload))!>0){
                Util.showToastCourse("打回成功！", context);
          ...
          ActionIconButton(
            ...
            tooltip: "提交批改",
            onPressed: (){
              DialogUtil.showConfirmDialog(context, "确认批阅完毕吗？", () async {
                Upload submitUpload = widget.upload;
                String json = jsonEncode(_controller.document.toDelta().toJson());
                submitUpload.content = json;
                submitUpload.appraise=_appraiseController.text.trim();
                submitUpload.criticism=_criticismController.text.trim();
                submitUpload.score=int.parse(_scoreController.text.trim());
                submitUpload.review="1";
                if((await ApiClientSchdedule.correct(submitUpload))!>0){
                Util.showToastCourse("提交成功！", context);
                Navigator.pop(context);
                }
              });
            },
